...
//模板路径,生成后自行修改
$template_edit = "admin/esf/esfdistrict_edit.tpl";
$template_list = "admin/esf/esfdistrict_list.tpl";
...
function add()
{
	global $smarty, $pdo, $template_edit,$circle_option,$borough_option;
	$area_text=array(
	'0'=>'否',
	'1'=>'是');
	$borough_text=array(
	...
	$ValueList['borough_text'] = $borough_option;
...
function index()
{
	...
	$orderValue = isset($_GET['order']) ? $_GET['order'] : 'asc';
	...
	$page=new Page($pageSize,$recordCount,$currentPage,$subPages,$page_info,2,true);
	...
	foreach ($res as $key => $item) {
		...
		if(!is_null($item['area'])){
			//片区
			$area_text=$code->getInfoByCode($item['area']);
...
function filter($Data)
{
	...
	if (array_key_exists('tube',$Data)) { $Data['tube'] = $Data['tube']; $ValueList['tube']= "{$Data['tube']}";}
