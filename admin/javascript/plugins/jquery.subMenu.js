...
$.fn.extend({
        submenu:function(opt,callback){
				...
				if (submenu.savestatus)//如果选择了保存cookie状态，则去读菜单状态
				{
					...
					//luodong添加代码************开始**************************************************
					menuIdCookie=$.cookie("subMenuId_on");
					$('#'+menuIdCookie).css('background-image','url(../../admin/images/submenu/bg_menu2.png)');
					//luodong添加代码************结束**************************************************
				...
				_this.find("div").each(function(i){	
					...
					$(div).find("span").eq(0).mouseup(function(e){
						...
						if (ParentDiv.attr("class")=="collapsed" || ParentDiv.attr("class")=="first-child collapsed"){//状态为隐藏时
							...
							ParentDiv.animate({height: 31},submenu.speed);
				...
				collapseOthers = function(me){
					...
					_this.find("div").not($(me)).not(".collapsed,first-child collapsed").each(function(){
							$(this).animate({height: 31},submenu.speed);
							$(this).addClass("collapsed");
					})
				};
				//记录菜单状态
...
var isclick=function(obj){
	var d = new Date();
	d.setTime(d.getTime() + (30 * 24 * 60 * 60 * 1000));
	$.cookie('subMenuId_on', obj.id, {expires: d});
	self.parent.leftFrame.location.reload();
	self.parent.mainFrame.location=obj;
};