...
*/
    ...
    function save()
    {
        ...
        if($userinfo['cnt']>0)
        {
            if($pdo->update($_POST,DB_PREFIX_DR.'user_drinfo','uid='.$uid))
            {
                page_prompt('修改信息成功！',true,'myinfo.html',3);
            }
            else
            {
                page_prompt('修改信息失败！',false,'myinfo.html',3);
            }
        }
        else
        {
            ...
            {
                page_prompt('添加信息成功！',true,'myinfo.html',3);
            }
            else
            {
                page_prompt('添加信息失败！',false,'myinfo.html',3);
            }
        }
    ...
    function resetpass()
    {
        ...
        $smarty->show('mydr/myinfo_resetpass.tpl');
    }
    ...
    function savepass()
    {
        ...
        if($pass['cnt']<1){
            ShowMsg('原始密码不正确','member_myinfo.php?action=resetpass',0,3000);exit();
        }
        ...
        ShowMsg('密码修改不成功!','login.html',0,3000);exit();
