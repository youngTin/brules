<?php /* Smarty version 2.6.22, created on 2013-07-16 10:08:44
         compiled from admin/set_config.tpl */ ?>
<?php require_once(SMARTY_CORE_DIR . 'core.load_plugins.php');
smarty_core_load_plugins(array('plugins' => array(array('function', 'html_options', 'admin/set_config.tpl', 44, false),)), $this); ?>
...
<form action="/admin/set_config.php?action=save" method="post">
	<div class="t">
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			<tr class="line">
				<td width="25%">网站标题</td>
				<td width="75%"><input name="config[title]" type="text" class="input" id="config[title]" value="<?php echo $this->_tpl_vars['configList']['db_title']; ?>
" size="50"></td>
			</tr>
			<tr class="line">
				<td>站点网址</td>
				<td><input name="config[url]" type="text" class="input" id="config[url]" value="<?php echo $this->_tpl_vars['configList']['db_url']; ?>
" size="50" />
					<span style="color:#FF0000">(最后面不要加/，请务必设置正确)</span></td>
			</tr>
			<tr class="line">
				<td>网站Email</td>
				<td><input name="config[contact]" type="text" class="input" id="config[contact]" value="<?php echo $this->_tpl_vars['configList']['db_contact']; ?>
" size="50"></td>
			</tr>
			<tr class="line">
				<td>联系我们Tel</td>
				<td><input name="config[tel]" type="text" class="input" id="config[tel]" value="<?php echo $this->_tpl_vars['configList']['db_tel']; ?>
" size="50"></td>
			</tr>
			<tr class="line">
				<td>网站ICP备案号</td>
				<td><input name="config[icp]" type="text" class="input" id="config[icp]" value="<?php echo $this->_tpl_vars['configList']['db_icp']; ?>
" size="50"></td>
			</tr>
			<tr class="line">
				<td>区县配置</td>
				...
				<select id="config[borough]" name="config[borough]">
					<?php echo smarty_function_html_options(array('options' => $this->_tpl_vars['borough'],'selected' => $this->_tpl_vars['configList']['db_borough']), $this);?>

				</select></td>
			</tr>
			<tr class="line">
				<td>DEBUG 模式运行网站</td>
				<td><input type="radio" name="config[debug]" value="1" <?php if ($this->_tpl_vars['configList']['db_debug'] == 1): ?>CHECKED<?php endif; ?> />
					开启
					<input type="radio" name="config[debug]" value="0" <?php if ($this->_tpl_vars['configList']['db_debug'] == 0): ?>CHECKED<?php endif; ?> />
					关闭 <br />
					(不屏蔽程序报错信息，同时开启数据库查询日志，但影响系统速度)</td>
			</tr>
			<tr class="line">
				<td>GZIP 压缩输出</td>
				<td><input type="radio" name="config[gzip]" value="1" <?php if ($this->_tpl_vars['configList']['db_gzip'] == 1): ?>CHECKED<?php endif; ?> />
					开启
					<input type="radio" name="config[gzip]" value="0" <?php if ($this->_tpl_vars['configList']['db_gzip'] == 0): ?>CHECKED<?php endif; ?>  />
					关闭 <br>
					(允许网站通过 gzip 输出页面,可以很明显的降低带宽需求,但需要客户端支持,并会加大服务器系统开销)</td>
			</tr>
			<tr class="line">
				<td>帮助消息框</td>
				<td><input type="radio" name="config[hidehelp]" value="0" <?php if ($this->_tpl_vars['configList']['db_hidehelp'] == 0): ?>CHECKED<?php endif; ?> />
					显示
					<input type="radio" name="config[hidehelp]" value="1"  <?php if ($this->_tpl_vars['configList']['db_hidehelp'] == 1): ?>CHECKED<?php endif; ?> />
					隐藏				</td>
			</tr>
			<tr class="line">
				<td>站点语言/编码</td>
				<td><select name="config[lang]" id="config[lang]">
						...
						<option value="utf-8" <?php if ($this->_tpl_vars['configList']['db_lang'] == 'utf-8'): ?>selected<?php endif; ?>>utf-8</option>
					</select></td>
			</tr>
			<tr class="line">
				<td>站点Meta关键字</td>
				<td>
				<input type="text" maxlength="255" name="config[metakeyword]" id="config[metakeyword]" size="60" value="<?php echo $this->_tpl_vars['configList']['db_metakeyword']; ?>
" class="form-text" /></td>
			</tr>
			<tr class="line">
			  <td>站点Meta描述</td>
			  <td>
			  <input type="text" maxlength="255" name="config[metadescrip]" id="config[metadescrip]" size="60" value="<?php echo $this->_tpl_vars['configList']['db_metadescrip']; ?>
" class="form-text" />
			  </td>
		    </tr>
			<tr class="line">
				<td>站点首页模板</td>
				<td><input name="config[template_index]" type="text" class="input" id="config[template_index]" value="<?php echo $this->_tpl_vars['configList']['db_template_index']; ?>
" size="50" />
			...
			<tr class="line">
				...
				<select id="config[default_tplpath]" name="config[default_tplpath]">
					...
					<option value="tpl/news" <?php if ($this->_tpl_vars['configList']['db_default_tplpath'] == 'tpl/news'): ?>selected<?php endif; ?>>news</option>
					<option value="tpl/user1" <?php if ($this->_tpl_vars['configList']['db_default_tplpath'] == 'tpl/user1'): ?>selected<?php endif; ?>>user1</option>
					<option value="tpl/user2" <?php if ($this->_tpl_vars['configList']['db_default_tplpath'] == 'tpl/user3'): ?>selected<?php endif; ?>>user2</option>
				</select></td>
			</tr>
			<tr class="line">
				<td>忽略Gif图片的缩略</td>
			...
			<tr class="line">
			  <td>网站尾部信息</td>
			  <td><div><input type="hidden" id="config[footer]" name="config[footer]" value="<?php echo $this->_tpl_vars['configList']['db_footer']; ?>
" style="display:none" />
...
-->
	...
	<option value="2" selected  >&raquo;新闻动态</option><option value="1" selected  >&raquo;培训项目</option><option value="6" selected  >&raquo;考试介绍</option><option value="3" selected  >&raquo;校园生活</option><option value="4" selected  >&raquo;公司相册</option><option value="9" >&raquo;求职就业</option><option value="10" >&raquo;开课通知</option><option value="7" >&raquo;友情链接</option><option value="11" >&raquo;最新公告</option><option value="12" >&raquo;附件下载</option>
			...
			<tr class="line">
				<td>是否开启图片水印功能</td>
				<td><input type="radio" name="config[ckwater]" value="1" <?php if ($this->_tpl_vars['configList']['db_ckwater'] == 1): ?>CHECKED<?php endif; ?> />
					开启
					<input type="radio" name="config[ckwater]" value="0"  <?php if ($this->_tpl_vars['configList']['db_ckwater'] == 0): ?>CHECKED<?php endif; ?> />
					关闭</td>
			</tr>
			<tr class="line">
				<td>使用水印类型</td>
				<td><input type="radio" name="config[watertype]" value="overlay" <?php if ($this->_tpl_vars['configList']['db_watertype'] == 'overlay'): ?>CHECKED<?php endif; ?> />
					图片水印
					<input type="radio" name="config[watertype]" value="text"  <?php if ($this->_tpl_vars['configList']['db_watertype'] == 'text'): ?>CHECKED<?php endif; ?> />
					文字水印</td>
			</tr>
			<tr class="line">
				<td>添加水印的图片大小控制</td>
				...
				<td>
					宽: <input size="5" name="config[waterwidth]" value="<?php echo $this->_tpl_vars['configList']['db_waterwidth']; ?>
" type="text" class="input">
					高:<input size="5" name="config[waterheight]" value="<?php echo $this->_tpl_vars['configList']['db_waterwidth']; ?>
" type="text" class="input">只对超过程序设置的大小的附件图片才加上水印图片或文字(设置为0不限制)
				</td>
			</tr>
			<tr class="line">
				<td>水印位置</td>
				<td><input name="config[waterpos]" value="0" type="radio" <?php if ($this->_tpl_vars['configList']['db_waterpos'] == 0): ?>CHECKED<?php endif; ?> />中心位置
					<input name="config[waterpos]" value="1" type="radio" <?php if ($this->_tpl_vars['configList']['db_waterpos'] == 1): ?>CHECKED<?php endif; ?> />顶部居左
					<input name="config[waterpos]" value="2" type="radio" <?php if ($this->_tpl_vars['configList']['db_waterpos'] == 2): ?>CHECKED<?php endif; ?> />底部居左
					<input name="config[waterpos]" value="3" type="radio" <?php if ($this->_tpl_vars['configList']['db_waterpos'] == 3): ?>CHECKED<?php endif; ?> />顶部居右
					<input name="config[waterpos]" value="4" type="radio" <?php if ($this->_tpl_vars['configList']['db_waterpos'] == 4): ?>CHECKED<?php endif; ?> />底部居右
				</td>
			</tr>
			<tr class="line">
				<td>水印图片文件名</td>
				<td><input type=text name="config[waterimg]" id="config[waterimg]" value="<?php echo $this->_tpl_vars['configList']['db_waterimg']; ?>
" class="input">
			...
			<tr class="line">
				<td>水印文字字体库</td>
				<td><input name="config[watertextlib]" id="config[watertextlib]" value="<?php echo $this->_tpl_vars['configList']['db_watertextlib']; ?>
" type="text" class="input">
			...
			<tr class="line">
				<td>水印图片文字</td>
				<td><input name="config[watertext]" value="<?php echo $this->_tpl_vars['configList']['db_watertext']; ?>
" type="text" class="input"></td>
			</tr>
			<tr class="line">
				<td>水印图片文字字体大小</td>
				<td><input name="config[waterfont]" value="<?php echo $this->_tpl_vars['configList']['db_waterfont']; ?>
" type="text" class="input"></td>
			</tr>
			<tr class="line">
				<td>水印图片文字颜色</td>
				<td><input name="config[watercolor]" value="<?php echo $this->_tpl_vars['configList']['db_watercolor']; ?>
" type="text" class="input">（十六进制颜色值，默认为#FF0000:红色)</td>
			</tr>
			
			<tr class="line">
				<td>水印透明度</td>
				<td><input name="config[waterpct]" value="<?php echo $this->_tpl_vars['configList']['db_waterpct']; ?>
" type="text" class="input">该值决定图片水印清晰度，其值范围从 0 到 100<br />当使用文字水印，其值从 0 到 127。0 表示完全不透明，127 表示完全透明</td>
			</tr>
			<!--
			<tr class="line">
				<td>图片质量</td>
				<td><input name="config[jpgquality]" value="<?php echo $this->_tpl_vars['configList']['db_jpgquality']; ?>
" type="text" class="input">该值决定 jpg 格式图片的质量，范围从 0 到 100</td>
			</tr>
			-->
			<!--
			<tr>
...
Total <?php echo $this->_tpl_vars['time']; ?>
(s) query 1 , Gzip disabled <br />
<br />
</div>
