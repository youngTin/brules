...
class BaseCode extends BasePage {
    ...
    */
    public function getBaseCodeByType($code_type, $show_name="base_code", $show_type="select", $rtn_type='string', $class_name="" ) {
		$aCode = $this->pdo->find(DB_PREFIX_HOME."code_basic", "flag=1 AND type=".$code_type,  "code, name");
    ...
    */
    public function getRegion($type='area', $select="") {
        ...
        // 代码查询
		$aCode = $this->pdo->find(DB_PREFIX_HOME."code_basic", $where,  "code, name", "code");
    ...
    * @return array 返回数组
    +----------------------------------------------------------
    */
	public function getPairBaseCodeByType($code_type, $where = "")
	{
		$aCode = $this->pdo->find(
			DB_PREFIX_HOME."code_basic", 
    ...
    */    
    public function getNameByCode($code) {
		$sql = "SELECT 
		 ...
		 WHERE 
		   code = %d AND flag <> 0";
		$sql = sprintf($sql, DB_PREFIX_HOME."code_basic", $code);
	...
	*/
	...
	{
		$sql = "Select * From ".DB_PREFIX_HOME."code_basic Where 1 and id={$id}";
        $arrcode = $this->pdo->getRow($sql);
	...
	*/
	...
	public function saveOrUpdateCode($data, $id)
	{
		...
		{
			if($id == 0){
				$this->pdo->add($data, DB_PREFIX_HOME."code_basic");
				return $this->pdo->getLastInsID();
			}else{
				$this->pdo->update($data , DB_PREFIX_HOME.'code_basic', "id={$id}");
				return $id;
			}
		}
	...
	*/
	...
	{		
		...
		if(1*$id > 0)
		{
			$isOk = $this->pdo->remove("id=".$id, DB_PREFIX_HOME."code_basic");
		}
	...
	public function removeMultiCode($ids)
	{
		...
		if(is_array($ids))
		{
			if(!empty($ids))
			{
				...
				$isOk = $this->pdo->remove("id in ('{$temp}')", DB_PREFIX_HOME."code_basic");
			}
		}
		else
		{
	...
	*/
	public function refreshCodeStatus($id)
	{
		if(1*$id > 0)
		{
			$isOk = $this->pdo->setField("flag", 0, DB_PREFIX_HOME."code_basic", "id=".$id);
		}
		return ($isOk ? "删除成功" : "删除失败");
	}
	...
	*/
	...
	{
		$info = $this->pdo->find(DB_PREFIX_HOME.'code_basic','flag = 1 and code = '.$code,'code,name');
