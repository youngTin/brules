 ...
 * */
	function login()
	{
		...
		if ($member->CheckUser($username,$password,$ajax))
		{
			...
			$member->SaveInfo($username,$member_type[$row['user_type']],$row['uid']);
			if ($member->IsLogin())
			{
                /*
				//增加积分时间限制
                ...
                */
				...
				//}
				if($ajax)exit("$uid");
				page_prompt("登录成功,3秒之后将进入会员中心..",true,'index.html',3);
				exit();
			}
			if($ajax)exit('-1');
			page_msg("登录失败!",true,'javascript:history.back(-1)',3);
			page_prompt("登录失败!",true,'javascript:history.back(-1)',3);
			exit();
		}
		else 
		{
