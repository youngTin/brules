...
function index(){
	...
	$smarty->assign('seoTitle','我的订单');
	$smarty->show('member/order_list.tpl');
}
...
function gopay(){
	...
	$smarty->show('member/gopay.tpl');
}

function confirm(){
	if(is_numeric($_POST['order_sn'])&&!empty($_POST['payway']))
	{
		...
		{
			require_once('./includes/pay/pay.config.php');
			if(array_key_exists($_POST['payway'],$payway)){//exit($_POST['payway']);
				
				require_once('./includes/pay/'.$_POST['payway'].'/pay.class.php');
				$pay = new pay();
				$pay->order_sn = $oinfo['order_sn'];
				$pay->name = '积分购买';
				$pay->totle_fee = '0.01'; //$oinfo['money'];
				$info = array('scores'=>$oinfo['scores'],'uid'=>uid);
				$pay->body = formatParameter($info,'in');
				$pay->show_url = '';
				echo $pay->go_url();
			}
...
function buy(){
	global  $smarty ; 
	$smarty->assign('ratio',10);
	$smarty->assign('seoTitle','积分购买');
	$smarty->show('member/order_buy.tpl');
}
...
function getorder(){
	...
	$time = time();
	$s = get_order_sn();
	$money = round($scores / 10,2);
	$sql = " insert into ".DB_PREFIX_HOME."user_order set uid = '".uid."' , order_sn = '$s' ,scores = '$scores' ,money = '$money' , status = '0' , addtime = '$time' ";
	if(getPdo()->execute($sql))
	{
		$sp = array('order_sn'=>$s);
