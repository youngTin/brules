...
<script src="{$smarty.const.URL}/admin/javascript/showDiv.js" type="text/javascript" language="javascript"></script>
</head>
{literal}
<style>
div#qTip {
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr class="tr4">
	...
	<input type="button" name="Submit5" value="设置本栏目" class="btn" onclick="window.location='/admin/category.php?action=edit&cid={$cid}&mid={$mid}';"/>
	</td>
  </tr>
</table>
</div>

<form name="form1" id="form1" method="post" action="/admin/column/content.php?action=option&cid={$cid}&mid={$mid}">
<div class="t">
<table width="100%" border="0" cellspacing="0" cellpadding="0">
   <tr class="head">
    <td colspan="14">
...
{section name=s loop=$contentindexList}
  <tr class="tr3">
    ...
    <td title="{$contentindexList[s].title}">{$contentindexList[s].title|default:"&nbsp;"|truncate_utf8:'20'}
  ...
  <img id='img{$smarty.section.s.index}_3' class='st' src='/admin/images/star_no.gif' onmouseover="showStar('{$smarty.section.s.index}','3');" onclick="digest('{$contentindexList[s].id}','{$smarty.section.s.index}','3');" />  </div>
  </div>
  ...
  <td>
  {if $contentindexList[s].ifpub==1}已审核{else}<font color="#FF0000">未审核</font>{/if}</td>
...
<a href="/admin/column/content.php?action=pubcancel&id={$contentindexList[s].id}&cid={$cid}&mid={$mid}"><img src="/admin/images/cancel.gif" align="absmiddle" alt="取消发布" /></a>
<a href="/admin/column/content.php?action=pubview&id={$contentindexList[s].id}&cid={$cid}&mid={$mid}"><img src="/admin/images/pub.gif" align="absmiddle" alt="发布内容" /></a>
</td>
  </tr>
{/section}
</table>
