...
<form name="form1" id="form1" method="post" action="/admin/column/content.php?action=order&cid=53&mid={$mid}" >
<div class="t">
<table width="100%" border="0" cellspacing="0" cellpadding="0">
   <tr class="head">
    <td colspan="14">
...
<tr class="tr2">
    <td><span id="ordertid" class="st">序号<span id="orderpostdate" class="st"><img src="/admin/images/order_DESC.gif" /></span></span></td>
    <td><span id="ordertitle" class="st">线路</span></td>
    <td><span id="orderpublisher" class="st">发布者/负责人</span></td>
...
{section name=s loop=$contentindexList}
  <tr class="tr3">
	...
	{if $contentindexList[s].photo!=''}<a href="javascript:void(0);" img="{$contentindexList[s].photo}"><img src="/admin/images/img.gif" /></a>{/if}</td>
  <td>{$contentindexList[s].publisher|default:"&nbsp;"}/{$contentindexList[s].username|default:"&nbsp;"}</td>
  <td>
  ...
  <img id='img{$smarty.section.s.index}_1' class='st' src='/admin/images/star_no.gif' onmouseover="showStar('{$smarty.section.s.index}','1');" onclick="digest('{$contentindexList[s].id}','{$smarty.section.s.index}','1');" alt="Fixed" />
  <img id='img{$smarty.section.s.index}_2' class='st' src='/admin/images/star_no.gif' onmouseover="showStar('{$smarty.section.s.index}','2');" onclick="digest('{$contentindexList[s].id}','{$smarty.section.s.index}','2');" alt="首页相册推荐" />
  <img id='img{$smarty.section.s.index}_3' class='st' src='/admin/images/star_no.gif' onmouseover="showStar('{$smarty.section.s.index}','3');" onclick="digest('{$contentindexList[s].id}','{$smarty.section.s.index}','3');" alt="首页Flash推荐" />  </div>
  </div>
  <script>reSet('{$smarty.section.s.index}','{$contentindexList[s].digest}');</script>  </td>
  <td><input name="taxis[{$contentindexList[s].id}]" type="text" class="input" id="taxis[{$contentindexList[s].id}]" value="{$contentindexList[s].comnum}" size="3" maxlength="2"></td>
  ...
  <td><a href="/admin/itinerary/itinerary.php?routeId={$contentindexList[s].id}">查看日程({$contentindexList[s].itineraryNum|default:'0'})</a></td>
  <td>
