...
<div class="br-total">
                        <div class="brt-01 brt-awd">
                            <span>处理违章<em>{$info.tnum}</em>宗，违章扣分<em>{$info.marking}</em>分， 罚金<em>{$info.fine}</em>元， 服务费<em>{$info.sefine}</em>元。</span>
                            <span class="brt-total">合计：<em id='t-total'>{$info.totalfine}</em>元</span>
                        </div>
                        <div class="clear"></div>
                    </div>
<p class="operation"></p>
