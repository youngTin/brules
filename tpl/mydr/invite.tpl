...
<div class="c-content invite">
    ...
    <div class="invite_znum">总邀请人数：<b class="gray">{$num}人</b></div>
    <div class="invite_notes">
        <div><ul><li class="in_title">邀请记录：</li>{foreach from=$ginfo item=item}<li>{$item.username}</li>{/foreach}</ul></div>
        <p>您的邀请奖励：每个好友<span class="red">{$smarty.const.INVITE_SCORES}</span>分</p>
    </div>
    <div class="invite_pri">
        <h3>您的专属链接</h3>
        <div class="ip_info">
            <p><input type="text" class="link" name="link" id="link" value="http://www.jiazhao888.com/i/{$info.uid}" /><button type="button" onclick="setClipboardfirst()" >复制</button></p>
            <p>复治上面的链接，通过QQ、旺旺、微博、论坛发帖等方式发送给好友，对方通过改链接注册即可~</p>
        </div>
    </div>
    <div class="invite_znum">总邀请收入：<b class="red">{$tgold}</b>元&nbsp;;&nbsp;&nbsp;可提现佣金：<b class="red">{$info.in_gold|default:0}</b>元&nbsp;&nbsp;&nbsp;</div>
